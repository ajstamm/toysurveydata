...
# set up ----
# load data settings
data_path <- "create_mock_data/data" # or whatever your path is
report_count <- 200
...
l$select_one <- select_categorical_set(d = vars, count = report_count) |>
  dplyr::mutate(
    res_check = sample(0:2, prob = c(50, 10, 1), replace = TRUE, report_count),
    res_region = ifelse(res_county %in% metro, "Urban", "Rural"),
    res_region = ifelse(is.na(res_county), NA, 
                 ifelse(res_check == 0, res_region, 
                 ifelse(res_check == 1, "I don't know", 
                 ifelse(res_region == "Urban", "Rural", "Urban"))))
  ) |> dplyr::select(-res_check)
