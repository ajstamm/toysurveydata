...
#' @param force_max   boolean; should all responses select the maximum number 
#'                    of options?
#' 
#' @examples
...
#' 
#' sample_many(var = "my_letters", d = d, count = 10, force_max = FALSE)
#' 
#' @export

sample_many <- function(var, d, count = 100, summary_var = FALSE,
                        force_max = FALSE) {
  ...
  if (!force_max) {
    t <- data.frame(id = 1:count,
                    v1 = sample_one(dt$options, dt$probability_1, count)) |>
      dplyr::mutate(v1 = ifelse(is.na(v1) | v1 == "", "delete", v1))
    
    if (max_opts > 1) {
      for (i in 2:max_opts) {
        excludes <- "refuse|other|none|^$"
        t <- dplyr::mutate(t, temp = sample_one(dt$options, dt$probability_2, 
                                                count),
    ...
    if (max_opts > 1) {
      for (i in 2:max_opts) {
        excludes <- "refuse|other|none|^$"
        vs <- names(t)[grepl("^v\\d", names(t))]
        
        t <- dplyr::rowwise(t) |>
          dplyr::mutate(remove = paste(dplyr::across(all_of(vs)), 
                                       collapse = "|")) 
        t <- dplyr::mutate(t, 
               temp = sample_one(dt$options[!grepl(remove, dt$options)], 
                                 dt$probability_2[!grepl(remove, dt$options)],
                                 count = 1, miss_pct = 0),
               temp = ifelse(grepl(excludes, v1) | v1 == temp | is.na(temp) |  
                             grepl(excludes, temp), "delete", temp),
               !!dplyr::sym(paste0("v", i)) := temp)
      }
    }
  }
  ...
  lbl <- data.frame(var = dt$options, lbl = dt$labels)
  ...
  if (summary_var) {
    tp <- tp |> dplyr::rowwise() |>
      dplyr::mutate(my_var = paste(dplyr::across(all_of(vars)[order(vars)]), 
                                   collapse = "; "),
                    my_var = gsub("NA; |; NA", "", my_var),
                    my_var = ifelse(my_var %in% c("NA", ""), NA, my_var))
    tp <- dplyr::rename(tp, !!dplyr::sym(var) := my_var)
  }
